...
class TestResearchStationLimitUI < Minitest::Test
  def setup
    ...
    @player_actions_js = File.read(File.join(__dir__, '..', 'public', 'js', 'player_actions.js'))
  end
  ...
  def test_build_click_handler_rejects_stale_clicks_at_research_station_limit
    assert_match(/gameState\.researchStations\?\.available\s*<=\s*0/, @player_actions_js)
    assert_match(/showInvalidActionMessage\('Maximum number of research stations reached\.'\)/, @player_actions_js)
  end
end
